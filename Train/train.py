...
# from diffusers import UNetSpatioTemporalConditionOutput
...
class DummyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # start_idx = sample = random.choice([0, 1, 30])
        ...
        for i, frame_name in enumerate(selected_frames):
            ...
            with Image.open(frame_path) as img:
                ...
                # print(img_resized.size, flush=True)
...
def tensor_to_vae_latent(t, vae):
    ...
    # vae.to('cuda:1')
    # end = time.time()
    # print(f'Time use: {end - start}')

    # original_device = t.device
    # vae_device = vae.device
    # print(f't device: {original_device}')
    ...
    # print_tensor_vram_usage(latents)
    # print(f'Latent Shape: {latents.shape}', flush=True)
    # latents = latents.to(original_device)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help="Folder to validation images`.",
    )
    parser.add_argument(
        "--validation_steps",
        type=int,
...
def main():
    args = parse_args()
    args.validation_images_folder = args.validation_images_folder if args.validation_images_folder else ''
    ...
    print(f'Acceleate Device: {accelerator.device}', '*' * 100)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # print('before pred:')
                # print_cuda_memory_usage(0, show=True)
                # print_cuda_memory_usage(1, show=True)

                # print(inp_noisy_latents.shape, flush=True)
                # print(timesteps.shape, flush=True)
                # print(encoder_hidden_states.shape, flush=True)
            ...
            # print('lr', lr_scheduler.get_lr(), flush=True)
                        ...
                        with torch.autocast(
                            ...
                            # for val_img_idx in (range(args.num_validation_images) if global_step >= 18000 else range(1)):
                                ...
                                video_frames = pipeline(
                                    load_image(f'{args.validation_images_folder}/demo{val_img_idx}.jpg').resize((args.width, args.height)),
                                    height=args.height,
                                    width=args.width,
